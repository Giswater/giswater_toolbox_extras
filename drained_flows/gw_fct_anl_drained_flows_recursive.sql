...
CREATE OR REPLACE FUNCTION ud_sample.gw_fct_anl_drained_flows_recursive( p_node character varying, p_row integer, p_intensity double precision)
  RETURNS json AS
...
BEGIN
	...
	SELECT ftrack_id, node_area, imperv INTO v_track_id, v_node_drained_area, v_imperv FROM anl_drained_flows_node WHERE node_id = p_node;
	...
	v_node_runoff_area = v_node_drained_area * v_imperv;
	v_node_runoff_flow = v_node_runoff_area * p_intensity;
	...
	IF (v_track_id = 0) THEN
		...
		FOR rec_arc IN SELECT arc_id, max_flow, node_1 FROM arc JOIN anl_drained_flows_node USING (arc_id) WHERE node_2 = p_node
		LOOP
			...
			IF ((v_max_capacity > 0.0) AND (rec_arc.max_flow > 0.0)) THEN

				-- Check flow availability for the other pipes
				IF ((v_num_outlet > 1) AND (v_max_capacity <> rec_arc.max_flow)) THEN 
				
					-- Node has flowregulator
					IF v_hasflowreg THEN
						
						-- Get weightweing parameters
						v_flowreg = (SELECT CASE WHEN flowreg IS TRUE THEN 0 ELSE 1 END FROM anl_drained_flows_arc WHERE arc_id = rec_arc.arc_id);
						...
						IF v_flowreg_initflow < v_ups_runoff_flow THEN
						...
						ELSE	
							...
							v_weight0 := (((rec_arc.max_flow + v_flowreg_initflow) / v_max_capacity) * (v_ups_runoff_flow - v_flowreg_initflow)) / v_ups_runoff_flow;
							
							-- factor for normal conduits
							v_weight1 := (((rec_arc.max_flow - v_flowreg_initflow) / v_max_capacity) * (v_ups_runoff_flow - v_flowreg_initflow) + v_flowreg_initflow) / v_ups_runoff_flow;
						
							-- Conduit as flow regulator
							IF v_flowreg = 0 THEN
							
								v_ups_drained_area := v_ups_drained_area * v_weight0;
								v_ups_runoff_area := v_ups_runoff_area * v_weight0;
								v_ups_runoff_flow := v_ups_runoff_flow * v_weight0;
										
							-- Conduit as normal flow
							ELSIF 
							
								v_ups_drained_area := v_ups_drained_area * v_weight1;
								v_ups_runoff_area := v_ups_runoff_area * v_weight1;
								v_ups_runoff_flow := v_ups_runoff_flow * v_weight1;
							END IF
						END IF;
						...
						-- There is no weightweing. All flow from node flows trough current arc					
						IF v_flowreg_initflow < v_ups_real_flow THEN
						...
						ELSE	
							...
							-- factor for flow regulators
							v_weight0 := (((rec_arc.max_flow + v_flowreg_initflow) / v_max_capacity) * (v_ups_real_flow - v_flowreg_initflow)) / v_ups_real_flow;
							
							-- factor for normal conduits
							v_weight1 := (((rec_arc.max_flow - v_flowreg_initflow) / v_max_capacity) * (v_ups_real_flow - v_flowreg_initflow) + v_ups_real_flow) / v_ups_real_flow;
						
							-- conduit as flow regulator
							IF v_flowreg = 0 THEN
								v_ups_real_flow := v_ups_real_flow * v_weight0;
										
							-- conduit as normal flow
							ELSIF 
								v_ups_real_flow := v_ups_real_flow * v_weight1;
							END IF
						END IF;
					...
					ELSE
						...
						v_ups_drained_area := (v_num_wet_outlet / v_num_outlet) * (rec_arc.max_flow / v_max_capacity) * v_ups_drained_area;
						v_ups_runoff_area := (v_num_wet_outlet / v_num_outlet) * (rec_arc.max_flow / v_max_capacity) * v_ups_runoff_area;
						v_ups_runoff_flow := (v_num_wet_outlet / v_num_outlet) * (rec_arc.max_flow / v_max_capacity) * v_ups_runoff_flow;
						v_ups_real_flow := (v_num_wet_outlet / v_num_outlet) * (rec_arc.max_flow / v_max_capacity) * v_ups_real_flow;

				-- If whole flow comes from upstream node to current arc (num outlet = 1 or v_max_capacity = rec_arc.max_flow)
				ELSE
			...
			IF (rec_arc.max_flow > 0) THEN 
				v_ups_real_flow := LEAST(v_ups_real_flow, rec_arc.max_flow);
			END IF;
